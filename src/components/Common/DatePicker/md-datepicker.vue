...
export default {
    ...
    nextMonth(type) {
      ...
      showDay(time) {
        if (time === undefined||!Date.parse(time)) {
          this.checked.currentMoment = moment()
            ...
            this.checked.currentMoment = moment(time, this.options.format)
        }
        this.showOne('day')
        ...
        for (let i = 0; i < firstDay - (this.options.startsOnSunday? 0 : 1); i++) {
            let passiveDay = {
      ...
      showCheck() {
        ...
        if (this.model == '' || !(this.model)) {
          ...
          this.dayMonth = this.model;
          this.checked.oldtime = this.model
        }

        this.showDay(this.model)
      ...
      picked(){
        ...
        this.dayMonth = moment(ctime, "YYYY-MM-DD HH:mm");
        this.model = moment(this.checked.currentMoment).format(this.options.format)
        this.showInfo.check=false
      },
      dismiss(evt){
        if(evt.target.className === 'datepicker-overlay'){
          if(this.options.dismissible == undefined || this.options.dismissible){
            this.showInfo.check = false;
          }
        }
    }
  ...
  filters:{
    getWeekdayMonth(date){
      ...
      if(!(this.options.startsOnSunday)){
        weekday = this.library.week[moment(date).day() -1 ]
      }else{
        weekday = this.library.week[moment(date).day()]
      }

      let month = this.library.month[moment(date).month()];
...
<template>
  <div class="cov-vue-date">
    ...
    <div class="datepicker-overlay"
      v-if="showInfo.check"
      @click="dismiss($event)"
      v-bind:style="{'background' : options.overlayOpacity? 'rgba(0,0,0,'+options.overlayOpacity+')' : 'rgba(0,0,0,0.5)'}">

      <div 
      class="cov-date-body non-selectable" 
      >

        <div class="cov-date-labels" :style="{'background-color':  '#3f51b5'}">
        ...
        <div class="cov-date-monthly">
          ...
          <div class="cov-date-caption" :style="{'color':  '#fff'}">
        ...
        <div class="button-box">
          <span class="cal-buttons" @click="showInfo.check=false">{{options.buttons? options.buttons.cancel : 'Cancel' }}</span>
          <span class="cal-buttons" @click="picked">{{options.buttons? options.buttons.ok : 'Ok'}}</span>
        </div>
      </div>
